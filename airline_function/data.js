  function createFlightCard(flight) {
    const WHATSAPP_NUMBER = "61477174571";
		const cardMessage = `Flight Inquiry ✈️
				From: ${flight.from}
				To: ${flight.to}
				Date: ${formatDate(flight.date)}
				Aircraft: ${flight.aircraft}
				Price: €${Number(flight.price).toLocaleString()}`;
		const whatsApp_url = `https://wa.me/${WHATSAPP_NUMBER}?text=${encodeURIComponent(cardMessage)}`;
  return `
  ...
  <div class="flight-card__body">
      ...
      <div class="card-btn-wrapper">
          <div class="card-btn-content">
                ...
                <a href="${whatsApp_url}" class="btn-wrapper">
                    <span class="btn-icon">
